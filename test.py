...
def age_gender_detector (input_path):
  ...
  return frame_face, age_predictions_list, gender_predictions_list

def plot_results(age_predictions_list, gender_predictions_list):
    plt.figure(figsize=(10, 5))
    sns.countplot(x=age_predictions_list, order=AGE_LIST)
    ...
    sns.countplot(x=gender_predictions_list, order=GENDER_LIST)
...
if __name__ == "__main__":
  output, age_predictions_list, gender_predictions_list = age_gender_detector(sys.argv[1])

  plot_results(age_predictions_list, gender_predictions_list)
